...
def install_module(request):
    ...
    test_tryton.install_module('payment_gateway_braintree')
...
def dataset(request):
    ...
    with Transaction().start(DB_NAME, USER, context=CONTEXT) as transaction:
        ...
        braintree_gateway = PaymentGateway(
            name='Credit Card - Braintree',
            journal=cash_journal,
            provider='braintree',
            method='credit_card',
            braintree_api_key="1384640ad456f37934cc365fb6773f17",
            braintree_public_key="yxr8yz65qxw87748",
            braintree_merchant_id="t3scq4k2ckwrxsnr",
            braintree_currency=usd,
            test=True
        )
        braintree_gateway.save()
